...
public class FirstPlayer extends Player {
    ...
    private long shootTime = 500000000;

    ImageHandler heartsImage = new ImageHandler("main/Graphics/hearts/lifebar_16x16.png", 16, 16);

    public FirstPlayer(ImageHandler imageHandler, Vector2D position, int sizeX, int sizeY, int currentDirection) {
    ...
    public void update(Player player) {
        ...
        for (Bullet bullet : SecondPlayer.bullets) {
    ...
    public void update() {
        animation();
        move();
        if (!collisions.CollisionTile(dX, 0)) {
            position.x += dX;
        }
        if (!collisions.CollisionTile(0, dY)) {
            position.y += dY;
        }
        shoot();
        for (Bullet bullet : bullets) {
    ...
    public void shoot() {
        if (shoot) {
            ...
            if (Math.abs(bullet.bulletSpawnTime - lastBulletTime) > shootTime) {
                bullets.add(bullet);
